...
function loadFromFile(svgFilePath, callback, options={}) {
	...
	if(xml.indexOf("<title>") > 0 && xml.indexOf("<MetaInfo>") <= 0) {
		xml = xml.substr(0, xml.indexOf("<title>")) + '<MetaInfo xmlns="http://www.prognoz.ru"><Geo><GeoItem X="-595.30" Y="-142.88" Latitude="37.375593" Longitude="-121.977795"/><GeoItem X="1388.66" Y=" 622.34" Latitude="37.369930" Longitude="-121.959404"/></Geo></MetaInfo>' + xml.substr(xml.indexOf("<title>"))
	}
		...
		const geo = {
			...
			features:layerByName[name].map(data => {
				...
				return {
					...
					properties:data.coordinates.properties,
					geometry:data
				};
			})
		};
...
function addPathToLayer(el, layer, xform) {
	...
	if (c.d) {
		...
		addProperties(geo, el, `path @ ${pathStart && pathStart[0]}`)
	}
}
...
function addRectToLayer(el, layer, xform) {
	...
	addProperties(geo, el, `rect @ ${c.x},${c.y}`);
	layer.push(geo);
}
...
function addLineToLayer(el, layer, xform) {
	...
	addProperties(geo, el, `line @ ${c.x1},${c.y1}`);
	layer.push(geo);
}
...
function addCircleToLayer(el, layer, xform) {
	...
	addProperties(geo, el, `circle @ ${c.cx},${c.cy}`);
}
...
function addPolygonToLayer(el, layer, xform) {
	...
	addProperties(geo, el, `polygon @ ${pts[0]},${pts[1]}`);
	layer.push(geo);
}
...
function addPolylineToLayer(el, layer, xform) {
	...
	addProperties(geo, el, `polyline @ ${pts[0]},${pts[1]}`);
	layer.push(geo);
}
...
function addProperties(geo, el, fallback) {
	let string = el.$.id.value
	let name = string.substring(0, string.indexOf('+'))
	let site = string.substring(string.indexOf('+') + 1)
